...
This will produce:
- `<Paper Title>.wav` — the full audio file, named after the first heading in the transcript (e.g., `Measuring Progress Toward AGI - A Cognitive Framework.wav`). Falls back to `audiobook.wav` if no heading is found. Use `--output` to override.
- `audiobook_meta.yaml` — metadata (duration, voice, chapters, `audio_file` field with the output filename)
