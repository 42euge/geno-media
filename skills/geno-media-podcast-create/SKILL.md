...
Inside the target folder you expect to find (output from `/geno-media-audiobook-create`):
...
- Read `FOLDER/audiobook_meta.yaml` for duration/chapter info and the `audio_file` field
- Verify the audio file exists (use the `audio_file` value from meta, fall back to `audiobook.wav`)
- Store the resolved audio filename as `AUDIO_FILE` for use in later steps

### 2. Convert audio for alignment

The alignment tool expects `.mp3`. Convert the wav:
```bash
ffmpeg -y -i "FOLDER/AUDIO_FILE" -codec:a libmp3lame -qscale:a 2 "FOLDER/audiobook.mp3"
```
...
- This skill is designed to run **after** `/geno-media-audiobook-create` has already generated the audio
...
- For a fully custom animated video, use `/geno-media-video-create` instead
- Uses the same `~/.geno-tools/geno-media/scripts/video/` tooling (align_audio.py, sync_utils.py, Manim) as `/geno-media-video-create`
